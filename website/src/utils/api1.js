function block(time) {
  ...
  function getData() {
    ...
    if (keys.length === 0) {
      return null; // No active monitors
    }
    ...
    for (let i = 0; i < rowsToGenerate; i++) {
      tableData.push({
        value: getRandomFloat(config.lsl, config.usl), // Use actual USL/LSL from settings
        MR: getRandomFloat(0, 10)         // Increased range for MR values
      });
    }

    // Calculate X-bar as average of values in tableData
    const xBar = tableData.reduce((acc, cur) => acc + cur.value, 0) / rowsToGenerate;
  
    // Calculate stdDev and avgMR
    ...
    const UCL_X = config.usl + (config.a2 * avgMR);
    const LCL_X = config.lsl - (config.a2 * avgMR);
    const UCL_MR = config.d4 * avgMR;
    const LCL_MR = config.d3 * avgMR;
