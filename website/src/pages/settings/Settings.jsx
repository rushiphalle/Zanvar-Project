...
function SettingBoard({ refreshAction, monitorCode, settingData }) {
  ...
  return (
    <div className={styles.boardContainer}>
      {shown ? <SettingForm refreshAction={refreshAction} monitorCode={monitorCode} usl={settingData.usl} lsl={settingData.lsl} sampleSize={settingData.sampleSize} machineName={settingData.machineName} machineIP={settingData.machineIP} toolOffsetNumber={settingData.toolOffsetNumber} offsetSize={settingData.offsetSize} closeEvent={() => setShown(false)} /> : <></>}
      ...
      <div className={styles.boardSettings}>
        ...
        <div><strong>Sample Size</strong> {settingData.sampleSize}</div>
...
function SettingForm({ refreshAction, monitorCode, usl, lsl, sampleSize, machineName, machineIP, toolOffsetNumber, offsetSize, closeEvent }) {
  ...
  const [sampleSizeState, setDatapointSize] = useState(sampleSize || "");
  ...
  const handleSubmit = async (e) => {
    ...
    let ack = await update(code, Number(uslState), Number(lslState), Number(sampleSizeState), machineNameState, machineIPState, Number(toolOffsetNumberState), Number(offsetSizeState));
    setSpinner(null);
    if (ack.code == 200) {
